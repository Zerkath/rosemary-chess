...
public class Commons {
  ...
  static boolean pieceMoveNotPossible(
      int rowOffset, int colOffset, Board board, Moves moves, Coordinate origin, boolean isWhite) {
    short destination =
    ...
    if (isWhite != Pieces.isWhite(target)) moves.add(origin, new Coordinate(destination));
    return true;
  }
}
