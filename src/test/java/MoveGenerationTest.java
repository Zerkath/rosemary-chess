...
public class MoveGenerationTest {
  ...
  void speedMiddleGame10000Legal() {
    BoardState boardState =
        new BoardState("r3k2r/pbpnnpbp/1p1pp1p1/7Q/7q/1P1PP1P1/PBPNNPBP/R3K2R w KQkq - 6 10");
    int iterations = 10000;
    for (int i = 0; i < iterations; i++) {
      moveGenerator.getLegalMoves(boardState);
    }
    ...
    for (int i = 0; i < depth.length; i++) {
      long[] result =
          uci.runPerft(
              i + 1,
              true,
              getTestBoard("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1"));
      depth[i] = result[0];
      System.out.println(
          "Depth: "
              + (i + 1)
  ...
  void movesFromPosition2() {
    ...
    for (int i = 0; i < depth.length; i++) {
      long[] result =
          uci.runPerft(i + 1, true, getTestBoard("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1"));
      depth[i] = result[0];
      System.out.println(
          "Depth: "
              + (i + 1)
    ...
    for (int i = 0; i < depth.length; i++) {
      long[] result =
          uci.runPerft(
              i + 1,
              true,
              getTestBoard("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1"));
      depth[i] = result[0];
      System.out.println(
          "Depth: "
              + (i + 1)
    ...
    for (int i = 0; i < depth.length; i++) {
      long[] result =
          uci.runPerft(
              i + 1,
              true,
              getTestBoard("r2q1rk1/pP1p2pp/Q4n2/bbp1p3/Np6/1B3NBn/pPPP1PPP/R3K2R b KQ - 0 1"));
      depth[i] = result[0];
      System.out.println(
          "Depth: "
              + (i + 1)
    ...
    for (int i = 0; i < depth.length; i++) {
      long[] result =
          uci.runPerft(
              i + 1,
              true,
              getTestBoard("rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8"));
      depth[i] = result[0];
      System.out.println(
          "Depth: "
              + (i + 1)
